...
# line-by-line process for one item
...
head(test) #nice, this works perfectly
```
...
```{r 10.13}
# object that contains all elements
...
however, we can reduce the redundancies to a minimum.

***Step 1.***
First import the data and create the custom functions that can be generalized to each item.

```{r eval=FALSE}
load("C:/Users/rschn/OneDrive/Research/Legal DM Lab stuff/Lab Studies/JLWOP/Data and Models/jlwop_reentry_survey.RData")
...
***Step 2.***
Create the object that contains all elements (a list in this case) of interest; then pass said object to map and supply the function.
```{r eval=FALSE}

dfs=list(na_blank=na_blank, na_zero=na_zero) %>% # create lists
  map(.f=rotate_data, variable_prefix = "barrier") %>% # apply custom function from above to each element in list
  map(rank_and_unpivot) # again, apply function to each element in list
...
To export all data sets at once with purrr instead of writing each
file individually, see the last section of this chapter.
...
3.  `pmap` The `pmap()` function is for iterating over *three or more*
...
### Read/Import several files at once with `map()`
...
### Exporting multiple .csv files at once with `walk2()`
